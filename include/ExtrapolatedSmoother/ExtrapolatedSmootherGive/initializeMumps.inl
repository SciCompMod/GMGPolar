...
void ExtrapolatedSmootherGive<LevelCacheType>::initializeMumpsSolver(DMUMPS_STRUC_C& mumps_solver,
                                                                     SparseMatrixCOO<double>& solver_matrix)
{
    ...
    for (int i = 0; i < solver_matrix.non_zero_size(); i++) {
        solver_matrix.increment_row_index(i);
        solver_matrix.increment_col_index(i);
    }
    ...
    if (mumps_solver.sym == SYM_POSITIVE_DEFINITE && INFOG(mumps_solver, 12) != 0) {
        std::cout << "Warning: ExtrapolatedSmoother inner boundary matrix is not positive definite: Negative pivots in "
...
template <class LevelCacheType>
void ExtrapolatedSmootherGive<LevelCacheType>::finalizeMumpsSolver(DMUMPS_STRUC_C& mumps_solver)
{
