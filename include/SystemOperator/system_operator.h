...
{
public:
    explicit SystemOperator(const PolarGrid& grid, const LevelCache& level_cache, const DomainGeometry& domain_geometry,
                            const DensityProfileCoefficients& density_profile_coefficients, const bool DirBC_Interior,
                            const int num_omp_threads);
    virtual ~SystemOperator() = default;

    virtual void apply(Vector<double>& result, const Vector<double>& x) const = 0;
