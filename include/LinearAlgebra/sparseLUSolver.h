...
// Default constructor
template <typename T>
SparseLUSolver<T>::SparseLUSolver() : factorized_(false) {}
 ...
 */
template <typename T>
void SparseLUSolver<T>::solveInPlacePermuted(double* b) const {
 ...
 */
template <typename T>
SparseMatrixCSR<T> SparseLUSolver<T>::permuteMatrix(const SparseMatrixCSR<T>& A, 
                                                   const std::vector<int>& perm, 
                                                   const std::vector<int>& perm_inv) const {
    const int n = A.rows();
    ...
    SparseMatrixCSR<T> A_perm(n, n, [&](int i) { return nz_per_row[i]; });
 ...
 */
template <typename T>
void SparseLUSolver<T>::symbolicFactorization(const SparseMatrixCSR<T>& A,
                                            std::vector<std::vector<int>>& L_pattern,
                                            std::vector<std::vector<int>>& U_pattern) const {
    const int n = A.rows();
    L_pattern.clear();
