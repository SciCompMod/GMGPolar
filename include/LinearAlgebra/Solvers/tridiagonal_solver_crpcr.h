...
public:
    ...
    BatchedTridiagonalSolverCRPCR(int matrix_dimension, int batch_count, bool is_cyclic = true,
        ...
        , num_cr_levels_(crpcr_exact_log2(
              crpcr_next_power_of_two(matrix_dimension) /
        ...
        , is_factorized_(false)
    {
        ...
        cr_k1_trajectory_ = Vector<T>("BatchedTridiagonalSolverCRPCR::cr_k1_trajectory",
                                       static_cast<std::size_t>(batch_count) *
                                           static_cast<std::size_t>(compact_length_));
        cr_k2_trajectory_ = Vector<T>("BatchedTridiagonalSolverCRPCR::cr_k2_trajectory",
                                       static_cast<std::size_t>(batch_count) *
                                           static_cast<std::size_t>(compact_length_));
        ...
        pcr_k1_trajectory_ = Vector<T>("BatchedTridiagonalSolverCRPCR::pcr_k1_trajectory",
                                        static_cast<std::size_t>(batch_count) *
                                            static_cast<std::size_t>(num_pcr_steps_) *
                                            static_cast<std::size_t>(intermediate_system_size_));
        pcr_k2_trajectory_ = Vector<T>("BatchedTridiagonalSolverCRPCR::pcr_k2_trajectory",
                                        static_cast<std::size_t>(batch_count) *
                                            static_cast<std::size_t>(num_pcr_steps_) *
                                            static_cast<std::size_t>(intermediate_system_size_));
                ...
                for (int L = 0; L < num_cr_levels; ++L) {
                    ...
                    for (int i = rank; i < n_padded; i += team_size) {
                        if ((i + 1) % stride == 0) {
                            ...
                            const std::size_t traj_idx =
                                static_cast<std::size_t>(batch_idx) * compact_length +
                                static_cast<std::size_t>(level_offsets(L)) + static_cast<std::size_t>(t);
    ...
    void solve(Vector<T> rhs, int batch_offset = 0, int batch_stride = 1) override
    {
        ...
        if (!is_cyclic) {
            const std::size_t scratch_elems = 2ull * static_cast<std::size_t>(n_padded) + 2ull * static_cast<std::size_t>(m);
                    ...
                    for (int L = num_cr_levels - 1; L >= 0; --L) {
                        ...
                        for (int i = rank; i < n_padded; i += team_size) {
                            if ((i + 1) % stride == half) {
                                ...
                                d_rhs[cur][i] = (d_rhs[cur][i] - a_i * d_rhs[cur][pLeft] - c_i * d_rhs[cur][pRight]) / b_i;
                                d_buf[cur][i] = (d_buf[cur][i] - a_i * d_buf[cur][pLeft] - c_i * d_buf[cur][pRight]) / b_i;
                            }
                        }
                        team.team_barrier();
                    }
    ...
    int matrixDimensionPadded() const { return matrix_dimension_padded_; }
    int intermediateSystemSize() const { return intermediate_system_size_; }
    int numCrLevels() const { return num_cr_levels_; }
    int numPcrSteps() const { return num_pcr_steps_; }
    int compactTrajectoryLength() const { return compact_length_; }
