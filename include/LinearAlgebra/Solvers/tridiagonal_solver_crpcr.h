 ...
 *  - The team-per-system Kokkos::TeamPolicy parallelization model (one team per batch entry,
 *    team scratch memory, team_barrier()-separated read/write phases within a step), with
 *    equations distributed across whatever team size Kokkos::AUTO selects via strided loops --
 *    the same "one team per system" structure as BatchedTridiagonalSolverPCR, just without
 *    requesting an exact team size (see the comment in setup() for why: not every backend
 *    supports an arbitrary exact team size, and correctness here never depends on team size
 *    matching any particular equation count).
...
public:
    ...
    void setup() override
    {
        ...
        // Team size: use Kokkos::AUTO rather than requesting n_padded as an *exact* team size.
        // Not every backend supports an arbitrary exact team size -- CPU backends (Serial,
        // OpenMP) only support team_size == 1, and requesting anything else fails outright
        // rather than degrading gracefully; even on GPU backends the runtime-chosen size need
        // not equal, divide, or be divided by n_padded. Conceptually every padded position
        // still "participates in the reduction" (section 3), but not necessarily via its own
        // dedicated thread: every loop below is strided by the *runtime* team size
        // (team_member.team_size()), so all n_padded positions are covered by however many
        // threads the team actually has -- including the degenerate team_size == 1 case, which
        // just makes each loop fully sequential within a single thread. Correctness does not
        // depend on team_size == n_padded; only parallel throughput does.
    ...
    void solve(Vector<T> rhs, int batch_offset = 0, int batch_stride = 1) override
    {
        ...
        if (!is_cyclic) {
            ...
            // See the corresponding comment in setup() -- Kokkos::AUTO, not an exact n_padded
            // request; every loop below is strided by the runtime team size.
