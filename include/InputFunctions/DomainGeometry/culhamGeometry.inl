...
KOKKOS_INLINE_FUNCTION double CulhamGeometry::Fx(double r, double theta) const
{
    ...
    return (r / Rmax) * cos_theta + Delta((r / Rmax)) - E((r / Rmax)) * cos_theta - P((r / Rmax)) * cos_theta +
           T((r / Rmax)) * cos_two_theta + 5.0;
}
...
KOKKOS_INLINE_FUNCTION double CulhamGeometry::Fy(double r, double theta) const
{
    ...
    return (r / Rmax) * sin_theta - E((r / Rmax)) * sin_theta - P((r / Rmax)) * sin_theta -
           T((r / Rmax)) * sin_two_theta;
}
...
KOKKOS_INLINE_FUNCTION double CulhamGeometry::dFx_dr(double r, double theta) const
{
    ...
    double cos_theta           = std::cos(theta);
    const double cos_two_theta = 1.0 - sin_theta * sin_theta;
    return (Delta_prime((r / Rmax)) - E_prime((r / Rmax)) * cos_theta + T_prime((r / Rmax)) * cos_two_theta -
            dP((r / Rmax)) * cos_theta + cos_theta) /
           Rmax;
}
...
KOKKOS_INLINE_FUNCTION double CulhamGeometry::dFy_dr(double r, double theta) const
{
    ...
    double cos_theta           = std::cos(theta);
    const double sin_two_theta = 2.0 * sin_theta * cos_theta;
    return ((-E_prime((r / Rmax))) * sin_theta - T_prime((r / Rmax)) * sin_two_theta - dP((r / Rmax)) * sin_theta +
            sin_theta) /
           Rmax;
}

// In earlier versions denoted by 'Jrt'
KOKKOS_INLINE_FUNCTION double CulhamGeometry::dFx_dtheta(double r, double theta) const
{
    double sin_theta           = std::sin(theta);
    double cos_theta           = std::cos(theta);
    const double sin_two_theta = 2.0 * sin_theta * cos_theta;
    return (-(r / Rmax)) * sin_theta + E((r / Rmax)) * sin_theta + P((r / Rmax)) * sin_theta -
           2.0 * T((r / Rmax)) * sin_two_theta;
}

// In earlier versions denoted by 'Jtt'
KOKKOS_INLINE_FUNCTION double CulhamGeometry::dFy_dtheta(double r, double theta) const
{
    double sin_theta           = std::sin(theta);
    double cos_theta           = std::cos(theta);
    const double cos_two_theta = 1.0 - sin_theta * sin_theta;
    return (r / Rmax) * cos_theta - E((r / Rmax)) * cos_theta - P((r / Rmax)) * cos_theta -
...
KOKKOS_INLINE_FUNCTION double CulhamGeometry::dg(double rr, double g) const
{
    return ((-g) *
            ...
            2.261946711816e-06 * (4.0 - 0.5 * (rr * rr)) / g) /
                                                           ...
                                                           double d_val) const
{
    return c * val / (rr * rr) -
           d_val * (pow(rr, (double)((-1))) + (4.0 - 0.5 * (rr * rr)) *
                                                  (2.0 * dg * rr / (4.0 - 0.5 * (rr * rr)) +
                                                   0.125 * g * (rr * rr) / pow((1.0 - 0.125 * (rr * rr)), 2.0) +
                                                   2.0 * g / (4.0 - 0.5 * (rr * rr))) /
                                                  (g * rr));
}
...
KOKKOS_INLINE_FUNCTION double CulhamGeometry::dP(double rr) const
{
    if (rr == 0) {
        ...
        return 0.015 * (rr * rr) + 0.1 * rr * Delta_prime(rr) + 0.1 * Delta(rr) - 0.2 * E(rr) * E_prime(rr) -
               2.0 * T(rr) * T_prime(rr) / rr + pow(T(rr), 2.0) / (rr * rr);
    }
}
