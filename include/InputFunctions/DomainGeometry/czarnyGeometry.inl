...
inline double CzarnyGeometry::Fx(double r, double theta) const
{
    ...
    return (1.0 -
            sqrt(1.0 + inverse_aspect_ratio_epsilon * (inverse_aspect_ratio_epsilon + 2.0 * (r / Rmax) * cos_theta))) /
           inverse_aspect_ratio_epsilon;
}
...
inline double CzarnyGeometry::Fy(double r, double theta) const
{
    ...
    double cos_theta = std::cos(theta);
    return ellipticity_e * factor_xi * (r / Rmax) * sin_theta /
           (1.0 + (1.0 - sqrt(1.0 + inverse_aspect_ratio_epsilon *
                                        (inverse_aspect_ratio_epsilon + 2.0 * (r / Rmax) * cos_theta))));
}
...
inline double CzarnyGeometry::dFy_dt(double r, double theta) const
{
    ...
    return (ellipticity_e * factor_xi * (r / Rmax) * cos_theta) / (2.0 - temp) -
           (ellipticity_e * factor_xi * inverse_aspect_ratio_epsilon * (r / Rmax) * (r / Rmax) * sin_theta *
            sin_theta) /
