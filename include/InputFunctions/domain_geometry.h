...
class Fx_Functor { // In earlier versions denoted by 'x'
public:
    explicit Fx_Functor(std::shared_ptr<TransformationHelper> helper);
    double operator()(const double& r, const double& theta, const double& sin_theta, const double& cos_theta) const;
private:
    std::shared_ptr<TransformationHelper> helper_;
};

class Fy_Functor { // In earlier versions denoted by 'y'
public:
    explicit Fy_Functor(std::shared_ptr<TransformationHelper> helper);
    double operator()(const double& r, const double& theta, const double& sin_theta, const double& cos_theta) const;
private:
    std::shared_ptr<TransformationHelper> helper_;
};
...
class dFx_dr_Functor { // In earlier versions denoted by 'Jrr'
public:
    explicit dFx_dr_Functor(std::shared_ptr<TransformationHelper> helper);
    double operator()(const double& r, const double& theta, const double& sin_theta, const double& cos_theta) const;
private:
    std::shared_ptr<TransformationHelper> helper_;
};

class dFy_dr_Functor { // In earlier versions denoted by 'Jtr'
public:
    explicit dFy_dr_Functor(std::shared_ptr<TransformationHelper> helper);
    double operator()(const double& r, const double& theta, const double& sin_theta, const double& cos_theta) const;
private:
    std::shared_ptr<TransformationHelper> helper_;
};

class dFx_dt_Functor { // In earlier versions denoted by 'Jrt'
public:
    explicit dFx_dt_Functor(std::shared_ptr<TransformationHelper> helper);
    double operator()(const double& r, const double& theta, const double& sin_theta, const double& cos_theta) const;
private:
    std::shared_ptr<TransformationHelper> helper_;
};

class dFy_dt_Functor { // In earlier versions denoted by 'Jtt'
public:
    explicit dFy_dt_Functor(std::shared_ptr<TransformationHelper> helper);
    double operator()(const double& r, const double& theta, const double& sin_theta, const double& cos_theta) const;
private:
    std::shared_ptr<TransformationHelper> helper_;
};

#include "domain_geometry.inl"