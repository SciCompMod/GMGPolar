...
void Operator::applyATake0(const Level& onLevel, Vector<scalar_t>& result, const Vector<scalar_t>& x) const{
    ...
    for(int index = 0; index < grid.number_of_nodes(); index ++){
        ...
        if(neighbors[0].second == -1){
            ...
            // beta_{s,t} / f_{s,t} //
            scalar_t value = (h1+h2)*(k1+k2) * coeff_beta * fabs(detDFinv) / 4  * x[grid.index(node)];
            ...
            // Right
            if(neighbors[0].second != -1){
                value += 0.5 * (k1 + k2) / h2 * (arr + arr_left) * x[index];
            }
            // Bottom
            if(neighbors[1].first != -1){
...
do { \
    ...
    if (i_r > 0 && i_r < grid.nr() - 2) { \
        ...
        \
        double coeff1 = 1/2*(k1+k2)/h1; \
        double coeff2 = 1/2*(k1+k2)/h2; \
        double coeff3 = 1/2*(h1+h2)/k1; \
        double coeff4 = 1/2*(h1+h2)/k2; \
        /* Fill result(i,j) */ \
        result[grid.index(i_r,i_theta)] += \
            (h1+h2)*(k1+k2) * coeff_beta * fabs(detDFinv) / 4 * x[grid.index(i_r,i_theta)] /* f_{i,j} */ \
    ...
    /* Node in the inner circle */ \
        ...
        /* Some more adjustments from the changing the 9-point stencil to a 7-point stencil. */ \
        ...
        /*  + art * x[grid.index(i_r,i_theta+1)] / 4 // Top Left: REMOVED DUE TO 7 POINT STENCIL */ \
        /*  - art * x[grid.index(i_r,i_theta-1)] / 4; // Bottom Left: REMOVED DUE TO 7 POINT STENCIL */ \
        /* Fill result(i+1,j) */ \
        result[grid.index(i_r+1,i_theta)] += \
            - coeff2 * arr * x[grid.index(i_r,i_theta)] /* Left */ \
            + coeff2 * arr * x[grid.index(i_r+1,i_theta)] /* Center: (Left) */ \
        ...
        result[grid.index(i_r,i_theta-1)] += \
            ...
            + coeff3 * att * x[grid.index(i_r,i_theta-1)] /* Center: (Top) */ \
            - art * x[grid.index(i_r+1,i_theta)] / 4; /* Top Right */ \
        /*  + art * x[grid.index(i_r-1,i_theta)] / 4; // Top Left: REMOVED DUE TO 7 POINT STENCIL */ \
        /* Fill result(i,j+1) */ \
        result[grid.index(i_r,i_theta+1)] += \
            - coeff4 * att * x[grid.index(i_r,i_theta)] /* Bottom */ \
            + coeff4 * att * x[grid.index(i_r,i_theta+1)] /* Center: (Bottom) */ \
            + art * x[grid.index(i_r+1,i_theta)] / 4; /* Bottom Right */ \
        /*  - art * x[grid.index(i_r-1,i_theta)] / 4; // Bottom Left: REMOVED DUE TO 7 POINT STENCIL */ \
        \
    ...
    /* Node in the outer circle */ \
        ...
        /* Don't forget to give values to the interior nodes! */ \
        scalar_t h1 = grid.r_dist(i_r-1); \
        scalar_t k1 = grid.theta_dist(i_theta-1); \
        scalar_t k2 = grid.theta_dist(i_theta); \
        \
