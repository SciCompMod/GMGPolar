...
int DirectSolver_COO_MUMPS_Take::getSolverMatrixIndex(const int i_r, const int i_theta) const
{
    ...
    if (i_r >= grid_.numberSmootherCircles() && i_r < grid_.nr() - 2) {
        ...
        const int prior_inner_boundary_nodes      = grid_.ntheta();
        const int prior_next_inner_boundary_nodes = grid_.ntheta();
        const int prior_interior_nodes            = grid_.ntheta() * (grid_.numberSmootherCircles() - 2) +
                                         i_theta * (grid_.lengthSmootherRadial() - 2) + i_r -
                                         grid_.numberSmootherCircles();
        ...
        const int prior_interior_nodes =
            grid_.ntheta() * (grid_.numberSmootherCircles() - 2) + (i_theta + 1) * (grid_.lengthSmootherRadial() - 2);
        const int prior_next_outer_boundary_nodes = i_theta;
        const int prior_outer_boundary_nodes      = i_theta;
        return size_stencil_inner_boundary * prior_inner_boundary_nodes +
               size_stencil_next_inner_boundary * prior_next_inner_boundary_nodes +
