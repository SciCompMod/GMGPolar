...
#define NODE_APPLY_SYSTEM_OPERATOR_TAKE(i_r, i_theta, grid, DirBC_Interior, result, x, arr, att, art, detDF,                            \
                                        coeff_beta)                                                                                     \
    do {                                                                                                                                \
        /* -------------------- */                                                                                                      \
...
void SystemOperatorTake::applyCircleSection(const int i_r, Vector<double>& result, const Vector<double>& x) const
{
    ...
    for (int i_theta = 0; i_theta < grid_.ntheta(); i_theta++) {
        NODE_APPLY_SYSTEM_OPERATOR_TAKE(i_r, i_theta, grid_, DirBC_Interior_, result, x, arr, att, art, detDF,
                                        coeff_beta);
    }
}

void SystemOperatorTake::applyRadialSection(const int i_theta, Vector<double>& result, const Vector<double>& x) const
{
    assert(level_cache_.cacheDensityProfileCoefficients());
    assert(level_cache_.cacheDomainGeometry());
