...
using namespace symmetric_matrixA_Stencil;

#define NODE_BUILD_SYMMETRIC_A_GIVE(i_r, i_theta, r, theta, sin_theta, cos_theta, \
    grid, DirBC_Interior, \
    matrixA, rhs, nz_index, \
...
do { \
    ...
    if (i_r > 1 && i_r < grid.nr() - 2) { \
        ...
        \
        /* Fill rhs of (i,j) */ \
        rhs[center_index] = 0.25 * (h1+h2)*(k1+k2) * (*rhs_f_)(r, theta, sin_theta, cos_theta) * fabs(detDF); \
        \
    ...
    /* ------------------------ */ \
        ...
        if(DirBC_Interior){ \
            /* Don't fill rhs of (i,j) here since we have done this at the start. */ \
            /* rhs[center_index] = (*u_D_Interior_)(r, theta, sin_theta, cos_theta); */ \
            /* Fill result(i,j) */ \
            scalar_t h2 = grid.r_dist(i_r); \
            scalar_t k1 = grid.theta_dist(i_theta-1); \
            scalar_t k2 = grid.theta_dist(i_theta); \
            ...
            \
            /* Don't give all values to the interior nodes of matrixA of row (i+1,j)! */ \
            /* Since we want to keep a symmetric matrix we remove */ \
            /* Left, Top Left and Bottom Left, but still keep Center(Left). */ \
            /* Instead shift these values to the rhs vector. */ \
            \
            /* Left, Top Left, Bottom Left */ \
            rhs[right_index] -= \
                - coeff2 * arr * rhs[center_index] + 0.25 * art * rhs[top_index] - 0.25 * art * rhs[bottom_index]; \
            \
            const Stencil& RightStencil = get_stencil(grid, i_r+1, DirBC_Interior); \
            \
            /* nz_index = right_nz_index + RightStencil[StencilType::Left]; */ \
    ...
    /* ---------------------------- */ \
        ...
        matrixA.value(nz_index) += 0.25 * (h1+h2)*(k1+k2) * coeff_beta * fabs(detDF); /* beta_{i,j} */ \
        \
        /* We move Left, Bottom Left and Top Left to the rhs to make matrixA symmetric. */ \
        rhs[center_index] -= - coeff1 * arr * rhs[left_index]; \
        /* nz_index = center_nz_index + CenterStencil[StencilType::Left]; */ \
        /* matrixA.row_index(nz_index) = center_index + 1; */ \
        /* matrixA.col_index(nz_index) = left_index + 1; */ \
        /* matrixA.value(nz_index) += - coeff1 * arr; // Left */ \
        ...
        \
        /* We move Left, Bottom Left and Top Left to the rhs to make matrixA symmetric. */ \
        rhs[bottom_index] -= 0.25 * art * rhs[left_index]; \
        /* nz_index = bottom_nz_index + BottomStencil[StencilType::TopLeft]; */ \
        /* matrixA.row_index(nz_index) = bottom_index + 1; */ \
        /* matrixA.col_index(nz_index) = left_index + 1; */ \
        /* matrixA.value(nz_index) += 0.25 * art; // Top Left */ \
        ...
        \
        /* We move Left, Bottom Left and Top Left to the rhs to make matrixA symmetric. */ \
        rhs[top_index] -= - 0.25 * art * rhs[left_index]; \
        /* nz_index = top_nz_index + TopStencil[StencilType::BottomLeft]; */ \
        /* matrixA.row_index(nz_index) = top_index + 1; */ \
        /* matrixA.col_index(nz_index) = left_index + 1; */ \
        /* matrixA.value(nz_index) += - 0.25 * art; // Bottom Left */ \
    ...
    /* ---------------------------- */ \
        ...
        matrixA.value(nz_index) += - coeff1 * arr; /* Left */ \
        \
        /* We move Right, Bottom Right and Top Right to the rhs to make matrixA symmetric. */ \
        rhs[center_index] -= - coeff2 * arr * rhs[right_index]; \
        /* nz_index = center_nz_index + CenterStencil[StencilType::Right]; */ \
        /* matrixA.row_index(nz_index) = center_index + 1; */ \
        /* matrixA.col_index(nz_index) = right_index + 1; */ \
        /* matrixA.value(nz_index) += - coeff2 * arr; // Right */ \
        ...
        \
        /* We move Right, Bottom Right and Top Right to the rhs to make matrixA symmetric. */ \
        rhs[bottom_index] -= - 0.25 * art * rhs[right_index]; \
        /* nz_index = bottom_nz_index + BottomStencil[StencilType::TopRight]; */ \
        /* matrixA.row_index(nz_index) = bottom_index + 1; */ \
        /* matrixA.col_index(nz_index) = right_index + 1; */ \
        /* matrixA.value(nz_index) += - 0.25 * art; // Top Right */ \
        ...
        \
        /* We move Right, Bottom Right and Top Right to the rhs to make matrixA symmetric. */ \
        rhs[top_index] -= 0.25 * art * rhs[right_index]; \
        /* nz_index = top_nz_index + TopStencil[StencilType::BottomRight]; */ \
        /* matrixA.row_index(nz_index) = top_index + 1; */ \
        /* matrixA.col_index(nz_index) = right_index + 1; */ \
        /* matrixA.value(nz_index) += 0.25 * art; // Bottom Right */ \
    ...
    /* ------------------------ */ \
        ...
        /* Fill rhs of (i,j) */ \
        /* Don't fill rhs of (i,j) here since we have done this at the start. */ \
        /* rhs[center_index] = (*u_D_)(r, theta, sin_theta, cos_theta); */ \
        \
        /* Fill matrix row of (i,j) */ \
        const Stencil& CenterStencil = get_stencil(grid, i_r, DirBC_Interior); \
        \
        ...
        /* Don't give all values to the interior nodes of matrixA of row (i-1,j)! */ \
        /* Since we want to keep a symmetric matrix we remove */ \
        /* Right, Top Right and Bottom Right, but still keep Center(Right). */ \
        /* Instead shift these values to the rhs vector. */ \
        \
        /* Right, Top Right, Bottom Right */ \
        rhs[left_index] -= \
            - coeff1 * arr * rhs[center_index] - 0.25 * art * rhs[top_index] + 0.25 * art * rhs[bottom_index]; \
        \
        const Stencil& LeftStencil = get_stencil(grid, i_r-1, DirBC_Interior); \
        \
        /* nz_index = left_nz_index + LeftStencil[StencilType::Right]; \
...
std::pair<SparseMatrix<scalar_t>, Vector<scalar_t>> Operator::build_symmetric_system(const Level& onLevel) const{
    ...
    #pragma omp parallel
    {
        ...
        for (int non_zero_index = 0; non_zero_index < matrixA_nnz; non_zero_index++){
            matrixA.value(non_zero_index) = 0.0;
        }
        assign(rhs, 0.0);
        #pragma omp for
        for (int i_theta = 0; i_theta < grid.ntheta(); i_theta++){
            const scalar_t sin_theta = sin_theta_[i_theta];
            const scalar_t cos_theta = cos_theta_[i_theta];
            const scalar_t theta = grid.theta(i_theta);
            rhs[grid.index(0, i_theta)] = DirBC_Interior_? (*u_D_Interior_)(grid.radius(0), theta, sin_theta, cos_theta) : 0.0;
            rhs[grid.index(1, i_theta)] = 0.0;
            rhs[grid.index(grid.nr()-2, i_theta)] = 0.0;
            rhs[grid.index(grid.nr()-1, i_theta)] = (*u_D_)(grid.radii().back(), theta, sin_theta, cos_theta);
        }
    }
    ...
    {   
        ...
        for (int i_r = i_r_end - i_r_separate; i_r < i_r_end; i_r++){
            ...
            for (int i_theta = 0; i_theta < grid.ntheta(); i_theta++){
                ...
                NODE_BUILD_SYMMETRIC_A_GIVE(i_r, i_theta, r, theta, sin_theta, cos_theta,
                    grid, DirBC_Interior_,
                    matrixA, rhs, nz_index,
                    center_nz_index, left_nz_index, right_nz_index, bottom_nz_index, top_nz_index,
                    center_index, left_index, right_index, bottom_index, top_index,
                    arr, att, art, coeff_beta, detDF);
    ...
    matrixA.write_to_file("matrixA.txt");
