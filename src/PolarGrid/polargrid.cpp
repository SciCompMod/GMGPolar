...
PolarGrid::PolarGrid(const std::string& file_grid_radii, const std::string& file_grid_angles, std::optional<double> splitting_radius)
{
...
PolarGrid::PolarGrid(const double& R0,
                     const double& Rmax,
                     const int nr_exp,
...
void PolarGrid::adjacentNeighborDistances(const MultiIndex& position, std::array<std::pair<double, double>, space_dimension>& neighbor_distance) const
{
    assert(position[0] >= 0 && position[0] < nr());
    assert(position[1] >= 0 && position[1] < ntheta());
...
void PolarGrid::adjacentNeighborsOf(const MultiIndex& position, std::array<std::pair<int, int>, space_dimension>& neighbors) const
{
    assert(position[0] >= 0 && position[0] < nr());
    assert(position[1] >= 0 && position[1] < ntheta());
...
void PolarGrid::diagonalNeighborsOf(const MultiIndex& position, std::array<std::pair<int, int>, space_dimension>& neighbors) const
{
    assert(position[0] >= 0 && position[0] < nr());
    assert(position[1] >= 0 && position[1] < ntheta());
