...
int main(int argc, char* argv[]){
    ...
    std::unique_ptr<ExactSolution> exact_solution = std::make_unique<CartesianR2_ShafranovGeometry>(Rmax, elongation_kappa, shift_delta);

    std::unique_ptr<DensityProfileCoefficients> coefficients = std::make_unique<SonnendruckerGyroCoefficients>(Rmax, alpha_jump);
    std::unique_ptr<BoundaryConditions> boundary_conditions = std::make_unique<CartesianR2_Boundary_ShafranovGeometry>(Rmax, elongation_kappa, shift_delta);
    std::unique_ptr<SourceTerm> source_term = std::make_unique<CartesianR2_SonnendruckerGyro_ShafranovGeometry>(Rmax, elongation_kappa, shift_delta);
    ...
    solver.multigrid_cycle(multigridCycle);
    ...
    solver.finestLevelThreads(finestLevelThreads);
    solver.threadReductionFactor(threadReductionFactor);

    int benchmark_size = 11;
