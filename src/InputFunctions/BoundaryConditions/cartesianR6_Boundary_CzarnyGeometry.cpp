...
CartesianR6_Boundary_CzarnyGeometry::CartesianR6_Boundary_CzarnyGeometry(const double& Rmax, const double& inverse_aspect_ratio_epsilon, const double& ellipticity_e) : 
    Rmax(Rmax),
...
double CartesianR6_Boundary_CzarnyGeometry::u_D(const double& r, const double& theta, const double& sin_theta, const double& cos_theta) const {
...
double CartesianR6_Boundary_CzarnyGeometry::u_D_Interior(const double& r, const double& theta, const double& sin_theta, const double& cos_theta) const {
    double temp = sqrt(inverse_aspect_ratio_epsilon * (2.0 * (r/Rmax) * cos_theta + inverse_aspect_ratio_epsilon) + 1.0);
    return 0.4096 * pow(((r/Rmax) - 1.0), 6.0) * 
        pow(((r/Rmax) + 1.0), 6.0) * 
        sin(2.0 * M_PI * ellipticity_e * (r/Rmax) * sin_theta * factor_xi / (2.0 - temp)) * 
