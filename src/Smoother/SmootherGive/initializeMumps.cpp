...
void SmootherGive::initializeMumpsSolver(DMUMPS_STRUC_C& mumps_solver, const SparseMatrix<double>& solver_matrix)
{
    ...
    mumps_solver.ICNTL(7) = 5; // Computes a symmetric permutation (ordering) to determine the pivot order to be used for the
    ...
    mumps_solver.ICNTL(25) = 0;   // Allows the computation of a solution of a deficient matrix and also of a null space basis
    ...
    mumps_solver.ICNTL(29) = 0;   // Defines the parallel ordering tool (when ICNTL(28)=1) to be used to compute the fill-in reducing permutation.
    ...
    mumps_solver.ICNTL(56) = 0; // Detects pseudo-singularities during factorization and factorizes the root node with a rankrevealing method
    ...
    mumps_solver.CNTL(5) = 0.0;  // Defines the fixation for null pivots and is effective only when null pivot row detection is active
    ...
    if (mumps_solver.sym == SYM_POSITIVE_DEFINITE && mumps_solver.INFOG(12) != 0) {
        std::cout<< "Warning: Smoother inner boundary matrix is not positive definite: Negative pivots in the factorization phase." << std::endl;
    }
}
