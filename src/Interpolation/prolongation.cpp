...
#pragma omp parallel for
    for (int index = 0; index < fineGrid.numberOfNodes(); index++)
    {
        ...
        if (fine_node[0] % 2 == 0 && fine_node[1] % 2 == 1)
        {
            ...
            result[index] = (k1 * x[coarseGrid.index(bottomNeighbor)] + k2 * x[coarseGrid.index(topNeighbor)]) / (k1 + k2);
        }
        ...
        if (fine_node[0] % 2 == 1 && fine_node[1] % 2 == 0)
        {
            ...
            result[index] = (h1 * x[coarseGrid.index(leftNeighbor)] + h2 * x[coarseGrid.index(rightNeighbor)]) / (h1 + h2);
        }
