...
void Interpolation::applyExtrapolatedProlongation0_(const PolarGrid& coarseGrid, const PolarGrid& fineGrid, Vector<double> result,
...
void Interpolation::applyExtrapolatedProlongation_(const PolarGrid& coarseGrid, const PolarGrid& fineGrid, Vector<double> result,
                                                  ConstVector<double> x, int nthreads) const
{
    assert(x.size() == static_cast<uint>(coarseGrid.numberOfNodes()));
    assert(result.size() == static_cast<uint>(fineGrid.numberOfNodes()));
