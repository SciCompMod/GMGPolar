...
do { \
    ...
    if (i_r > 0 && i_r < numberSmootherCircles-1) { \
        ...
        \
        int i_theta_M1 = grid.wrap_theta_index(i_theta-1); \
        int i_theta_P1 = grid.wrap_theta_index(i_theta+1); \
        \
    ...
    /* ------------------------------------------- */ \
        ...
        if(i_r & 1){ \
            if(i_theta & 1){ \
                ...
                auto& center_matrix = circle_symmetric_cyclic_tridiagonal_solver[i_r/2]; \
                auto& left_matrix = circle_diagonal_solver[(i_r-1)/2]; \
                auto& right_matrix = radial_symmetric_tridiagonal_solver[i_theta/2]; \
                \
                /* Fill matrix row of (i,j) */ \
                row = center_index; \
                column = center_index; \
...
void ExtrapolatedSmoother::build_Asc_matrices()
{
    ...
    #pragma omp parallel num_threads(maxOpenMPThreads_) if(grid_.number_of_nodes() > 100'000)
    {
        ...
        for (int circle_Asc_index = 0; circle_Asc_index < numberSmootherCircles; circle_Asc_index++){
            ...
            /* Interior Circle Section */
                ...
                if(circle_Asc_index & 1){
                    ...
                    auto& solverMatrix = circle_diagonal_solver_[circle_diagonal_solver_index];

                    solverMatrix = DiagonalSolver<double>(num_circle_nodes);

                    for (int i = 0; i < num_circle_nodes; i++) {
                        solverMatrix.diagonal(i) = 0.0;
                    }
                }
            }
        }
        ...
        for (int radial_Asc_index = 0; radial_Asc_index < grid_.ntheta(); radial_Asc_index++){
            ...
            if(radial_Asc_index & 1){
                ...
                auto& solverMatrix = radial_symmetric_tridiagonal_solver_[radial_tridiagonal_solver_index];
                ...
                auto& solverMatrix = radial_diagonal_solver_[radial_diagonal_solver_index];

                solverMatrix = DiagonalSolver<double>(num_radial_nodes);
