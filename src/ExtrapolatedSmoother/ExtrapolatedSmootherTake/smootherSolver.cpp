...
void ExtrapolatedSmootherTake::applyAscOrthoCircleSection(const int i_r, const SmootherColor smoother_color, const Vector<double>& x, const Vector<double>& rhs, Vector<double>& temp){
    assert(i_r >= 0 && i_r < grid_.numberSmootherCircles());
    ...
    for (int i_theta = 0; i_theta < grid_.ntheta(); i_theta++){
        NODE_APPLY_ASC_ORTHO_CIRCLE_TAKE(i_r, i_theta,
...
void ExtrapolatedSmootherTake::applyAscOrthoRadialSection(const int i_theta, const SmootherColor smoother_color, const Vector<double>& x, const Vector<double>& rhs, Vector<double>& temp){
    ...
    for (int i_r = grid_.numberSmootherCircles(); i_r < grid_.nr(); i_r++){
        NODE_APPLY_ASC_ORTHO_RADIAL_TAKE(i_r, i_theta,
            grid_, DirBC_Interior_, smoother_color, x, rhs, temp,
            arr, att, art, detDF, coeff_beta);
    }
}
...
void ExtrapolatedSmootherTake::solveCircleSection(const int i_r, Vector<double>& x, Vector<double>& temp, Vector<double>& solver_storage_1, Vector<double>& solver_storage_2) {
...
void ExtrapolatedSmootherTake::solveRadialSection(const int i_theta, Vector<double>& x, Vector<double>& temp, Vector<double>& solver_storage) {
...
void ExtrapolatedSmootherTake::extrapolatedSmoothingInPlace(Vector<double>& x, const Vector<double>& rhs, Vector<double>& temp) {
