...
__global__ void extrapolated_adjustAscCircle_ShermanMorrison_kernel(
...
void ExtrapolatedSmootherTakeGPU::adjustAscCircle_ShermanMorrison()
{
    const PolarGrid& grid = level_.grid();

    int blockSize = 256; 
    int numBlocks = (grid.numberSmootherCircles()/2 + blockSize - 1) / blockSize;

    extrapolated_adjustAscCircle_ShermanMorrison_kernel<<<numBlocks, blockSize>>>(
