                ...
                if (current_level - 1 == 0 && (extrapolation_ != ExtrapolationType::NONE))
                {
                    switch (FMG_cycle_)
                    {
                        case MultigridCycleType::V_CYCLE:
                            implicitlyExtrapolatedMultigrid_V_Cycle(current_level - 1, next_FMG_level.solution(), next_FMG_level.rhs(),
                                                                    next_FMG_level.residual());
                            break;

                        case MultigridCycleType::W_CYCLE:
                            implicitlyExtrapolatedMultigrid_W_Cycle(current_level - 1, next_FMG_level.solution(), next_FMG_level.rhs(),
                                                                    next_FMG_level.residual());
                            break;

                        case MultigridCycleType::F_CYCLE:
                            implicitlyExtrapolatedMultigrid_F_Cycle(current_level - 1, next_FMG_level.solution(), next_FMG_level.rhs(),
                                                                    next_FMG_level.residual());
                            break;
        ...
        for (int i_r = 0; i_r < grid.numberSmootherCircles(); i_r++)
        {
            ...
            for (int i_theta = 0; i_theta < grid.ntheta(); i_theta++)
            {
                ...
                error[grid.index(i_r, i_theta)] =
                    exact_solution_->exact_solution(r, theta, sin_theta, cos_theta) - solution[grid.index(i_r, i_theta)];
            }
        }
        #pragma omp for nowait
        for (int i_theta = 0; i_theta < grid.ntheta(); i_theta++)
