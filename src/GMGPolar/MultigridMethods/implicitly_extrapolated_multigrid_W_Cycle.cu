...
void GMGPolar::implicitlyExtrapolatedMultigrid_W_Cycle(const int level_depth, GPU_Vector<double>& solution,
                                                       GPU_Vector<double>& rhs, GPU_Vector<double>& residual)
{
    assert(0 <= level_depth && level_depth < number_of_levels_ - 1);
    ...
    if (level_depth + 1 == number_of_levels_ - 1) {
        ...
        // P_ex^T (f_l - A_l*u_l)
        level.computeResidual(residual, rhs, solution);
        extrapolatedRestriction(level_depth, next_level.GPU_residual(), residual);
        if(next_level.processingType() == ProcessingType::CPU_HYBRID){
            copyDeviceToHost(next_level.GPU_residual(), next_level.residual());
        } 
        // f_{l-1} - A_{l-1}* Inject(u_l)
        injection(level_depth, next_level.GPU_solution(), solution);
        if(next_level.processingType() == ProcessingType::CPU_HYBRID){
            copyDeviceToHost(next_level.GPU_solution(), next_level.solution());
        } 
        if(next_level.processingType() == ProcessingType::GPU){
            next_level.computeResidual(next_level.GPU_error_correction(), next_level.GPU_rhs(), next_level.GPU_solution());
        } else {
            next_level.computeResidual(next_level.error_correction(), next_level.rhs(), next_level.solution());
        }
        // res_ex = 4/3 * P_ex^T (f_l - A_l*u_l) - 1/3 * (f_{l-1} - A_{l-1}* Inject(u_l))
        if(next_level.processingType() == ProcessingType::GPU){
            linear_combination(next_level.GPU_residual(), 4.0 / 3.0, next_level.GPU_error_correction(), -1.0 / 3.0);
        } else {
            linear_combination(next_level.residual(), 4.0 / 3.0, next_level.error_correction(), -1.0 / 3.0);
        }
        auto end_MGC_residual = std::chrono::high_resolution_clock::now();
        ...
        // P_ex^T (f_l - A_l*u_l)
        level.computeResidual(residual, rhs, solution);
        extrapolatedRestriction(level_depth, next_level.GPU_error_correction(), residual);
        if(next_level.processingType() == ProcessingType::CPU_HYBRID){
        ...
        // res_ex = 4/3 * P_ex^T (f_l - A_l*u_l) - 1/3 * (f_{l-1} - A_{l-1}* Inject(u_l))
        if(next_level.processingType() == ProcessingType::GPU){
            linear_combination(next_level.GPU_error_correction(), 4.0 / 3.0, next_level.GPU_residual(), -1.0 / 3.0);
        } else {
            linear_combination(next_level.error_correction(), 4.0 / 3.0, next_level.residual(), -1.0 / 3.0);
        }
        auto end_MGC_residual = std::chrono::high_resolution_clock::now();
        ...
        /* Step 3: Solve for the error by recursively calling the multigrid cycle. */
        if(next_level.processingType() == ProcessingType::GPU){
            multigrid_W_Cycle(level_depth + 1, next_level.GPU_residual(), next_level.GPU_error_correction(), next_level.GPU_solution());
            multigrid_W_Cycle(level_depth + 1, next_level.GPU_residual(), next_level.GPU_error_correction(), next_level.GPU_solution());
        } else {
            multigrid_W_Cycle(level_depth + 1, next_level.residual(), next_level.error_correction(), next_level.solution());
            multigrid_W_Cycle(level_depth + 1, next_level.residual(), next_level.error_correction(), next_level.solution());
    ...
    extrapolatedProlongation(level_depth + 1, residual, next_level.GPU_residual());
