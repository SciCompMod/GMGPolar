...
void GMGPolar::setup()
{
    ...
    // Start with all threads at level 0, then reduce as depth increases
    double thread_reduction_factor = 1.0;
    for (int level_depth = 0; level_depth < number_of_levels_; ++level_depth) {
        // Compute reduced thread count and clamp it between 1 and max_omp_threads_
        int threads = static_cast<int>(std::floor(max_omp_threads_ * std::pow(thread_reduction_factor, level_depth)));
        threads_per_level_[level_depth] = std::max(1, std::min(max_omp_threads_, threads));
    }
