...
echo "#SBATCH --output=slurm-%A-FLOPS_DP_CARA.out" >> run_COMPACT_FLOPS_DP_likwid.sh
echo "#SBATCH --error=slurm-%A-FLOPS_DP_CARA.err" >> run_COMPACT_FLOPS_DP_likwid.sh
...
echo "#SBATCH --time=0:30:00" >> run_COMPACT_FLOPS_DP_likwid.sh
echo "#SBATCH --exclusive" >> run_COMPACT_FLOPS_DP_likwid.sh
echo "#SBATCH --nodelist=\"be-cpu02, be-cpu03, be-cpu04\"" >> run_COMPACT_FLOPS_DP_likwid.sh

echo "" >> run_COMPACT_FLOPS_DP_likwid.sh

echo "# potentially run benchmark on machine" >> run_COMPACT_FLOPS_DP_likwid.sh
echo "# srun --cpus-per-task=$((cores)) likwid-bench -i 1000 -t stream_avx_fma -w S0:500MB:56" >> run_COMPACT_FLOPS_DP_likwid.sh
...
echo "#SBATCH --output=slurm-%A-FLOPS_DP_CARA.out" >> run_COMPACT_MEM_DP_likwid.sh
echo "#SBATCH --error=slurm-%A-FLOPS_DP_CARA.err" >> run_COMPACT_MEM_DP_likwid.sh
...
echo "#SBATCH --time=0:30:00" >> run_COMPACT_MEM_DP_likwid.sh
echo "#SBATCH --exclusive" >> run_COMPACT_MEM_DP_likwid.sh
echo "#SBATCH --nodelist=\"be-cpu02, be-cpu03, be-cpu04\"" >> run_COMPACT_MEM_DP_likwid.sh


echo "" >> run_COMPACT_MEM_DP_likwid.sh

echo "# potentially run benchmark on machine" >> run_COMPACT_MEM_DP_likwid.sh
echo "# srun --cpus-per-task=$((cores)) likwid-bench -i 1000 -t stream_avx_fma -w S0:500MB:56" >> run_COMPACT_MEM_DP_likwid.sh
