...
TEST(GMGPolar_Extrapoaltion, NoExtrapolation)
{
    /* PolarGrid */
    double R0                              = 1e-8;
    double Rmax                            = 2.0;
    ...
    EXPECT_LE(number_of_iterations, 11);
    EXPECT_LE(exact_error_weighted_euclidean.value(), 2e-3);
    EXPECT_LE(exact_einfinity_error.value(), 6e-3);
}

TEST(GMGPolar_Extrapoaltion, ImplicitExtrapolation)
{
    /* PolarGrid */
    double R0                              = 1e-8;
    double Rmax                            = 2.0;
    ...
    EXPECT_LE(number_of_iterations, 45);
    EXPECT_LE(exact_error_weighted_euclidean.value(), 2e-4);
    EXPECT_LE(exact_einfinity_error.value(), 2e-3);
}

TEST(GMGPolar_Extrapoaltion, CombinedExtrapolation)
{
    /* PolarGrid */
    double R0                              = 1e-8;
    double Rmax                            = 2.0;
    ...
    ExtrapolationType extrapolation   = ExtrapolationType::COMBINED;
    int maxLevels                     = 7;
    int preSmoothingSteps             = 1;
    int postSmoothingSteps            = 1;
    MultigridCycleType multigridCycle = MultigridCycleType::V_CYCLE;
    ...
    EXPECT_LE(number_of_iterations, 37);
    EXPECT_LE(exact_error_weighted_euclidean.value(), 2e-4);
    EXPECT_LE(exact_einfinity_error.value(), 2e-3);
}
