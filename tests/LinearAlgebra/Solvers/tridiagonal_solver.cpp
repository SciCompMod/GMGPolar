...
// =================================================================================================
// New: matrix_dimension_ in the thousands. None of the tests above go past n=1024, which is
// already at the edge of what the O(n^3) dense reference can check in reasonable time. These use
// runHugeCorrectnessCheck (O(n) banded reference) to go substantially further, e.g. n=4000 - large
// enough that num_steps_ = ceil(log2(n)) reaches into the low teens and every backend's team_size
// is essentially guaranteed to be << n, so the strided team loops iterate many times per thread.
// =================================================================================================

template <typename SolverType>
void test_huge_matrix_dimension_non_cyclic()
{
    runHugeCorrectnessCheck<SolverType>(/*n=*/4000, /*batch_count=*/2, /*is_cyclic=*/false, /*offset=*/0,
                                         /*stride=*/1);
}
INSTANTIATE_FOR_ALL_SOLVERS(huge_matrix_dimension_non_cyclic)

template <typename SolverType>
void test_huge_matrix_dimension_cyclic()
{
    runHugeCorrectnessCheck<SolverType>(/*n=*/4000, /*batch_count=*/2, /*is_cyclic=*/true, /*offset=*/0,
                                         /*stride=*/1);
}
INSTANTIATE_FOR_ALL_SOLVERS(huge_matrix_dimension_cyclic)

// --- Huge, non-power-of-two matrix_dimension_ (4001 instead of 4000): checks that the "overshot"
// last reduction step seen in test_non_power_of_two_dimensions above still behaves correctly when
// num_steps_ is much larger, not just at small n.
template <typename SolverType>
void test_huge_non_power_of_two_matrix_dimension()
{
    for (bool is_cyclic : {false, true}) {
        runHugeCorrectnessCheck<SolverType>(/*n=*/4001, /*batch_count=*/2, is_cyclic, /*offset=*/0, /*stride=*/1);
    }
}
INSTANTIATE_FOR_ALL_SOLVERS(huge_non_power_of_two_matrix_dimension)

// --- Huge matrix_dimension_ combined with an uneven batch_offset_/batch_stride_ split, sharing a
// single setup() across multiple solve() calls - the n=4000-scale counterpart of
// test_very_large_matrix_dimension_with_batch_stride_* above.
template <typename SolverType>
void test_huge_matrix_dimension_with_batch_stride()
{
    const int matrix_dimension = 4000;
    const int batch_count      = 5;
    ...
    for (bool is_cyclic : {false, true}) {
        ...
        Kokkos::parallel_for(
            "FillHugeStride", 1, KOKKOS_LAMBDA(const int) {
                for (int b = 0; b < batch_count; b++) {
                    for (int i = 0; i < matrix_dimension; i++) {
                        solver.set_main_diagonal(b, i, sysDiag(b, i));
                    }
        ...
        for (int b = 0; b < batch_count; b++) {
            ...
            if (is_cyclic) {
                bandedReferenceSolveCyclic(matrix_dimension, diag, subdiag, sysCorner(b), rhs_ref);
            } else {
                bandedReferenceSolveNonCyclic(matrix_dimension, diag, subdiag, rhs_ref);
            }
...
INSTANTIATE_FOR_ALL_SOLVERS(huge_matrix_dimension_with_batch_stride)