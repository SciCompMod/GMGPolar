...
TEST(ExtrapolatedProlongationTest, ExtrapolatedProlongationMatchesStencil)
{
    ...
    HostVector<double> coarse_values = generate_random_sample_data(h_coarse_grid, 1234, 0.0, 1.0);
    ...
    for (int i_r = 0; i_r < fine_grid.nr(); ++i_r) {
        for (int i_theta = 0; i_theta < fine_grid.ntheta(); ++i_theta) {
            double expected = expected_extrapolated_value(h_coarse_grid, h_fine_grid, coarse_values, i_r, i_theta);
            double got      = fine_result[h_fine_grid.index(i_r, i_theta)];
