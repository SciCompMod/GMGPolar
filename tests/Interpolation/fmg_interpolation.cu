...
namespace FMGInterpolationTest
{
    ...
    TEST(FMGInterpolationTest, applyFMGInterpolation)
    {
        ...
        interpolation_operator.applyFMGInterpolation(coarse_level, finest_level, result, x);
        interpolation_operator.applyFMGInterpolation(coarse_level, finest_level, gpu_result, gpu_x);
        ...
        for (size_t i = 0; i < result.size(); ++i) {
            EXPECT_NEAR(result[i], host_result[i], precision);
        }
    }
}
