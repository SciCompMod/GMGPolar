...
TEST(Test_prolongation, Test_bilinear_prolongation)
{
    ...
    for (int z = 0; z < p_level.mc; z++) {
        u_test[z] = 1 - z + pow(PI, -z);
    }

    std::vector<double> sol = p_level.apply_prolongation_bi0(u_test, p_level.mc, p_level.m, p_level.coarse_nodes_list_r,
...
//TODO: Test for prolongation_bi not bi0 and add for P_ex and other
